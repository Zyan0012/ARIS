...
- **REVIEWER_MODEL = `gpt-5.4`** — Model used via `codex-review` MCP for slide review.
...
### Phase 5: `codex-review` MCP Review
...
> ⚠️ If `mcp__codex-review__review_start` is not available (no OpenAI API key), skip external review and proceed to Phase 6. Note the skip in `SLIDES_STATE.json`.
