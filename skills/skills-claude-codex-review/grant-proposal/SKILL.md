...
allowed-tools: Bash(*), Read, Write, Edit, Grep, Glob, WebSearch, WebFetch, Agent, Skill, mcp__codex-review__review, mcp__codex-review__review_reply, mcp__codex-review__review_start, mcp__codex-review__review_reply_start, mcp__codex-review__review_status
---
...
- **REVIEWER_MODEL = `gpt-5.4`** — Model used via `codex-review` MCP for proposal review. Must be an OpenAI model (e.g., `gpt-5.4`, `o3`, `gpt-4o`).
...
> ⚠️ **`codex-review` MCP fallback**: If `mcp__codex-review__review_start` is not available (no OpenAI API key), skip external review. Note "External review skipped — no `codex-review` MCP available. Consider running `/auto-review-loop-llm` separately." in GRANT_REVIEW.md. The proposal is still usable without external review.
...
```
mcp__codex-review__review_reply_start:
...
Implement CRITICAL and MAJOR fixes. If MAX_REVIEW_ROUNDS > 1, re-submit for another round via `mcp__codex-review__review_reply_start`.
...
| `reviewer model` | gpt-5.4 | Codex review model | → `codex-review` MCP |
