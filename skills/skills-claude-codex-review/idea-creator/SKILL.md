...
allowed-tools: Bash(*), Read, Write, Grep, Glob, WebSearch, WebFetch, Agent, mcp__codex-review__review, mcp__codex-review__review_reply, mcp__codex-review__review_start, mcp__codex-review__review_reply_start, mcp__codex-review__review_status
---
...
- **REVIEWER_MODEL = `gpt-5.4`** — Model used via `codex-review` MCP for brainstorming and review. Must be an OpenAI model (e.g., `gpt-5.4`, `o3`, `gpt-4o`).
...
Use the external LLM via `codex-review` MCP for divergent thinking:
...
2. **Critical review**: Use GPT-5.4 via `mcp__codex-review__review_reply_start` (same thread):
   ```
